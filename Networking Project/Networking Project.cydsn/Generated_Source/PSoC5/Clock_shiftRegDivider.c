/*******************************************************************************
* File Name: Clock_shiftRegDivider.c
* Version 2.20
*
*  Description:
...
#include "Clock_shiftRegDivider.h"
...
/*******************************************************************************
* Function Name: Clock_shiftRegDivider_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_shiftRegDivider_Start(void) 
{
    /* Set the bit to enable the clock. */
    Clock_shiftRegDivider_CLKEN |= Clock_shiftRegDivider_CLKEN_MASK;
	Clock_shiftRegDivider_CLKSTBY |= Clock_shiftRegDivider_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_shiftRegDivider_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_shiftRegDivider_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    Clock_shiftRegDivider_CLKEN &= (uint8)(~Clock_shiftRegDivider_CLKEN_MASK);
	Clock_shiftRegDivider_CLKSTBY &= (uint8)(~Clock_shiftRegDivider_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_shiftRegDivider_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_shiftRegDivider_StopBlock(void) 
{
    if ((Clock_shiftRegDivider_CLKEN & Clock_shiftRegDivider_CLKEN_MASK) != 0u)
    {
...
#if defined(Clock_shiftRegDivider__CFG3)
        CLK_DIST_AMASK = Clock_shiftRegDivider_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_shiftRegDivider_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_shiftRegDivider__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_shiftRegDivider_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_shiftRegDivider_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_shiftRegDivider_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_shiftRegDivider_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        Clock_shiftRegDivider_CLKSTBY &= (uint8)(~Clock_shiftRegDivider_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: Clock_shiftRegDivider_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_shiftRegDivider_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = Clock_shiftRegDivider_GetSourceRegister();
    uint16 oldDivider = Clock_shiftRegDivider_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_shiftRegDivider_CLKEN & Clock_shiftRegDivider_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Clock_shiftRegDivider_DIV_PTR, clkDivider);
                Clock_shiftRegDivider_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Clock_shiftRegDivider_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Clock_shiftRegDivider_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_shiftRegDivider_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Clock_shiftRegDivider_DIV_PTR, clkDivider);
				Clock_shiftRegDivider_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Clock_shiftRegDivider_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 Clock_shiftRegDivider_GetDividerRegister(void) 
{
    return CY_GET_REG16(Clock_shiftRegDivider_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_shiftRegDivider_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_shiftRegDivider_SetModeRegister(uint8 modeBitMask) 
{
    Clock_shiftRegDivider_MOD_SRC |= modeBitMask & (uint8)Clock_shiftRegDivider_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_shiftRegDivider_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_shiftRegDivider_ClearModeRegister(uint8 modeBitMask) 
{
    Clock_shiftRegDivider_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_shiftRegDivider_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_shiftRegDivider_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_shiftRegDivider_GetModeRegister(void) 
{
    return Clock_shiftRegDivider_MOD_SRC & (uint8)(Clock_shiftRegDivider_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_shiftRegDivider_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_shiftRegDivider_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = Clock_shiftRegDivider_GetDividerRegister();
    uint8 oldSrc = Clock_shiftRegDivider_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_shiftRegDivider_MOD_SRC |= CYCLK_SSS;
        Clock_shiftRegDivider_MOD_SRC =
            (Clock_shiftRegDivider_MOD_SRC & (uint8)(~Clock_shiftRegDivider_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_shiftRegDivider_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_shiftRegDivider_GetSourceRegister(void) 
{
    return Clock_shiftRegDivider_MOD_SRC & Clock_shiftRegDivider_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_shiftRegDivider_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void Clock_shiftRegDivider_SetPhaseRegister(uint8 clkPhase) 
{
    Clock_shiftRegDivider_PHASE = clkPhase & Clock_shiftRegDivider_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_shiftRegDivider_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 Clock_shiftRegDivider_GetPhaseRegister(void) 
{
    return Clock_shiftRegDivider_PHASE & Clock_shiftRegDivider_PHASE_MASK;
}
